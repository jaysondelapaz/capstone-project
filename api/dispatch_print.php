 ...
 <script src="../js/sweetalert.min.js"></script><?php
session_start();
	...
	try
	{
		...
		else
		{
			...
			$loadPrintQry = $mysql->prepare("SELECT ticket.ticketNo as ticketNo, ticket.upassenger as upassenger,ticket.divisioN as divisioN, ticket.udestination as udestination, ticket.udate as udate, ticket.utime as utime,ticket.Status as Status,ticket.Remarks as Remarks,ticket.Purpose as Purpose, ticket.Approvedby as Approvedby, CONCAT(driver.DfirstName,' ',driver.DlastName)as DriverName, vehicle.Vname as VehicleName, vehicle.VplateNo as PlateNo FROM tblticket as ticket LEFT JOIN tblemployee as driver ON ticket.EmployeeID = driver.EmployeeID LEFT JOIN tblvehicle as vehicle ON ticket.vehicleID = vehicle.vehicleID WHERE ticket.ticketNo ='$ID'");
				...
				$get = $mysql->prepare("SELECT upassenger FROM tblticket WHERE  ticketNo='$ID' ");
				$get->execute();
				$numCount = 0;
				while($rowName	 = $get->fetch(PDO::FETCH_OBJ))
					{
						...
						$pdf->Ln(5);
						$pdf->SetFont('Arial','',10);
						$pdf->Cell(72,5,$numCount.'. '.' '.' '. $rowName->upassenger,1,0,'L');
						$pdf->Cell(35,5,$division,1,0,'C');
						$pdf->Cell(40,5,$Remarkss,1,0,'C');
						$pdf->Cell(48,5,'',1,0,'C');
					}
				
				$userget = $mysql->prepare("SELECT emp.DfirstName as UfirstName,emp.DlastName as UlastName,user.EmployeeID as EmployeeID FROM tbluser as user inner join tblemployee as emp on user.EmployeeID = emp.EmployeeID WHERE user.uname=:UserName and user.EmployeeID=:EmpID ");
				$userget->bindParam(':UserName',$_SESSION['userName']);
				$userget->bindParam(':EmpID',$_SESSION['userEmployeeID']);
				$userget->execute();
				while($urow=$userget->fetch(PDO::FETCH_OBJ))
				{
					$userFullname = $urow->UfirstName.' '.$urow->UlastName;
					$userID=$urow->EmployeeID;
				}
				$pdf->Ln(47);
				$pdf->SetFont('Arial','I',10);
				$pdf->GetY(150);
				//$pdf->Write('C','Printed By: '.$userFullname);
