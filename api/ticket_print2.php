...
echo $_GET['datefrom']. $_GET['dateto'].$_GET['ticketNumber'].$_GET['passengerName'].$_GET['DriverID'].$_GET['VehicleID'];
	
			try
			{
			$loadPrintQry = $mysql->prepare("SELECT ticket.ticketNo as ticketNo, ticket.upassenger as upassenger,ticket.divisioN as divisioN, ticket.udestination as udestination, ticket.udate as udate, ticket.utime as utime,ticket.Status as Status,ticket.Remarks as Remarks,ticket.Purpose as Purpose, ticket.Approvedby as Approvedby, CONCAT(driver.DfirstName,' ',driver.DlastName)as DriverName, vehicle.Vname as VehicleName, vehicle.VplateNo as PlateNo FROM tblticket as ticket LEFT JOIN tblemployee as driver ON ticket.EmployeeID = driver.EmployeeID LEFT JOIN tblvehicle as vehicle ON ticket.vehicleID = vehicle.vehicleID");
				...
				$pdf->Ln(5);
				$pdf->SetFont('Arial','',8);
				$pdf->Cell(82,5,'NAME OF PASSENGER/S',1,0,'C');
				...
				//$pdf->Cell(197,5,'Printed By: ',0,0,'C');
				$pdf->Output();
				ob_end_flush(); 
